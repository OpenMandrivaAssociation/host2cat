BEGIN TRANSACTION; CREATE TABLE admins_backup(admid INTEGER PRIMARY KEY AUTOINCREMENT, aname TEXT UNIQUE, passwd TEXT, type TEXT NOT NULL); INSERT INTO admins_backup (admid, aname, passwd, type) SELECT admid, aname, passwd, type FROM admins; DROP TABLE admins; CREATE TABLE admins (admid INTEGER PRIMARY KEY AUTOINCREMENT, aname TEXT UNIQUE, passwd TEXT, type TEXT NOT NULL); INSERT INTO admins (admid, aname, passwd, type) SELECT admid, aname, passwd, type FROM admins_backup; DROP TABLE admins_backup; COMMIT;
BEGIN TRANSACTION; CREATE TABLE hierarchy_backup (id INTEGER PRIMARY KEY, parent INTEGER NOT NULL, child INTEGER NOT NULL); INSERT INTO hierarchy_backup (parent, child) SELECT parent, child FROM hierarchy; DROP TABLE hierarchy; CREATE TABLE hierarchy (id INTEGER PRIMARY KEY, parent INTEGER NOT NULL, child INTEGER NOT NULL); INSERT INTO hierarchy (id, parent, child) SELECT id, parent, child FROM hierarchy_backup; DROP TABLE hierarchy_backup; COMMIT;
CREATE TABLE categories (cat integer NOT NULL PRIMARY KEY, cname varchar(255) NOT NULL, type varchar(1) NOT NULL);
CREATE TABLE domains_categories (id integer NOT NULL PRIMARY KEY, did integer NOT NULL REFERENCES domains (did), cat integer NOT NULL REFERENCES categories (cat));
CREATE TABLE domains (did integer NOT NULL PRIMARY KEY, dname varchar(255) NOT NULL UNIQUE);
...
INSERT INTO categories (cat, cname, type) VALUES (200, "Uncategorized", "g");
...
INSERT INTO categories (cat, cname, type) VALUES (24, "Юридические услуги", "g");
...
INSERT INTO categories (cat, cname, type) VALUES (39, "Наука", "g");
